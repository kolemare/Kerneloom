...
namespace kl
{
    ...
    namespace
    {
        ...
        __global__ void backward_avgpool2d_cuda_float32_kernel(
        ...
        const Pooling2dOptions &options)
    {
        ...
        check_cuda(
            cudaGetLastError(),
            "CUDA backward avgpool2d zero kernel launch failed");
        ...
        backward_avgpool2d_cuda_float32_kernel<<<grid, block>>>(
            grad_output_data,
            grad_input_data,
            batch_size,
            channels,
        ...
        check_cuda(
            cudaGetLastError(),
            "CUDA backward avgpool2d kernel launch failed");

        check_cuda(
            cudaDeviceSynchronize(),
            "CUDA backward avgpool2d synchronization failed");
    }
