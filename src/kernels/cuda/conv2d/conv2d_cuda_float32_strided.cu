...
namespace kl
{
    namespace
    {
        ...
        __global__ void conv2d_cuda_float32_strided_kernel(
            const float *input,
            const float *kernels,
            const float *bias,
            float *result,
    ...
    void conv2d_cuda_float32_strided(
        const Tensor &input,
        ...
        const Conv2dOptions &options)
    {
        ...
        conv2d_cuda_float32_strided_kernel<<<grid, block>>>(
            input_data,
            kernel_data,
            bias_data,
            result_data,
        ...
        check_cuda(cudaGetLastError(), "CUDA conv2d strided kernel launch failed");
        check_cuda(cudaDeviceSynchronize(), "CUDA conv2d strided synchronization failed");
    }
