...
namespace kl
{
    namespace
    {
        ...
        __global__ void conv2d_cuda_float32_valid_kernel(
            const float *input,
            const float *kernels,
            const float *bias,
            float *result,
            ...
            bool use_bias)
        {
            ...
            for (std::size_t c = 0; c < C; ++c)
            {
                ...
                for (std::size_t r = 0; r < R; ++r)
                {
                    ...
                    {
                        sum += input[input_row_offset + ow + s] *
    ...
    void conv2d_cuda_float32_valid(
        const Tensor &input,
        ...
        const Conv2dOptions &options)
    {
        ...
        conv2d_cuda_float32_valid_kernel<<<grid, block>>>(
            input_data,
            kernel_data,
            bias_data,
            result_data,
