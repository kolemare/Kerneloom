...
namespace kl
{
    ...
    namespace
    {
            ...
            const Conv2dOptions &options)
        {
            ...
            {
                throw std::runtime_error("backward_conv2d stride must be greater than zero");
            }

            if (options.dilation_h == 0 || options.dilation_w == 0)
            {
                throw std::runtime_error("backward_conv2d dilation must be greater than zero");
            }
            ...
            {
                throw std::runtime_error("backward_conv2d expects tensors on the same device");
            }

            if (grad_bias != nullptr &&
                grad_bias->device().type() != input.device().type())
            {
                throw std::runtime_error("backward_conv2d expects grad_bias on the same device");
            }
            ...
            {
                throw std::runtime_error("backward_conv2d expects tensors with the same dtype");
            }

            if (grad_bias != nullptr &&
                grad_bias->dtype() != input.dtype())
            {
                throw std::runtime_error("backward_conv2d expects grad_bias with the same dtype");
            }

            if (input.dtype() != DType::Float32)
            {
                throw std::runtime_error("backward_conv2d currently supports only Float32 tensors");
            }

            if (input.rank() != 4)
            {
                throw std::runtime_error("backward_conv2d expects input shape N x C x H x W");
            }

            if (weights.rank() != 4)
            {
                throw std::runtime_error("backward_conv2d expects weights shape OUT x IN x KH x KW");
            }

            if (grad_output.rank() != 4)
            {
                throw std::runtime_error("backward_conv2d expects grad_output shape N x OUT x OH x OW");
            }

            if (grad_input.rank() != 4)
            {
                throw std::runtime_error("backward_conv2d expects grad_input shape N x C x H x W");
            }

            if (grad_weights.rank() != 4)
            {
                throw std::runtime_error("backward_conv2d expects grad_weights shape OUT x IN x KH x KW");
            }
            ...
            {
                throw std::runtime_error("backward_conv2d currently supports only RowMajor tensors");
            }

            if (grad_bias != nullptr &&
                grad_bias->storage() != Storage::RowMajor)
            {
                throw std::runtime_error("backward_conv2d currently supports only RowMajor grad_bias");
            }
            ...
            {
                throw std::runtime_error("backward_conv2d currently supports only NCHW layout");
            }
            ...
            const std::size_t weight_input_channels = weights.shape()[1];
            ...
            {
                throw std::runtime_error("backward_conv2d input channel mismatch");
            }

            if (grad_input.shape() != input.shape())
            {
                throw std::runtime_error("backward_conv2d grad_input shape must match input shape");
            }

            if (grad_weights.shape() != weights.shape())
            {
                throw std::runtime_error("backward_conv2d grad_weights shape must match weights shape");
            }

            const std::size_t expected_output_h = conv2d_output_size(
                input_h,
                kernel_h,
                options.padding_h,
                options.stride_h,
            ...
            const std::size_t expected_output_w = conv2d_output_size(
                input_w,
                kernel_w,
                options.padding_w,
                options.stride_w,
            ...
            {
                throw std::runtime_error("backward_conv2d grad_output shape mismatch");
            }
            ...
            if (grad_bias != nullptr)
            {
                ...
                {
                    throw std::runtime_error("backward_conv2d grad_bias shape mismatch");
                }
            }
        }
        ...
        const Conv2dOptions &options)
    {
        ...
        case DeviceType::CUDA:
            ...
            return;
#else
            throw std::runtime_error("CUDA backward_conv2d requested but CUDA backend is not enabled");
#endif
        ...
        case DeviceType::ROCM:
            ...
            return;
#else
            throw std::runtime_error("ROCm backward_conv2d requested but ROCm backend is not enabled");
#endif

        default:
            throw std::runtime_error("unknown DeviceType in backward_conv2d");
        }
    }
